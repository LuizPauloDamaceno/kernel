...
#define CURRENT_LIMIT_TO_2300_DURING_DCP_CHARGING
						...
						enum power_supply_type type)
{
	...
	if (type == POWER_SUPPLY_TYPE_USB)
		...
		if (chip->typec_current_ma > CURRENT_2300_MA)
			/* Flow chart: C-7 */
		...
		if (chip->typec_current_ma > CURRENT_2300_MA)
			/* Flow chart: C-3 */
#ifndef CURRENT_LIMIT_TO_2300_DURING_DCP_CHARGING
			current_limit_ma = somc_chg_get_typec_current_ma(chip,
							chip->typec_current_ma);
#else
